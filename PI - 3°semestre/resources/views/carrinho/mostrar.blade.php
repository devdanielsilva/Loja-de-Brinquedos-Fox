<!-- resources/views/carrinho/mostrar.blade.php -->
<!DOCTYPE html>
<html lang="pt-br">
        ...
        <div class="row row-width" id="header">
            ...
            <div class="col feature-box">
                ...
                <li class="nav-item dropdown">
                    ...
                    <ul class="dropdown-menu">
                        <li><a class="dropdown-item" href="../register">Minha Conta</a></li>
    ...
    <main>
        ...
        <div class="container mt-4">
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>R$ {{ number_format($item['produto']->PRODUTO_PRECO * $item['quantidade'], 2, ',', '.') }}
                        </td>
                        <td>
                            <form method="POST" action="{{ route('carrinho.remover') }}">
                                @csrf
                                <input type="hidden" name="produto_id" value="{{ $item['produto']->PRODUTO_ID }}">
            ...
            <div class="row">
                ...
                <div class="col-md-3">
                    <div class="card">
                        @if ($item['produto']->Produto_imagem->isNotEmpty())
                        <img src="{{ $item['produto']->Produto_imagem->first()->IMAGEM_URL }}" class="card-img-top" alt="{{ $item['produto']->PRODUTO_NOME }}">
                        @else
                        ...
                        <div class="card-body">
                            ...
                            <h5 class="card-title">{{ $item['produto']->PRODUTO_ID }}</h5>
                            <p class="card-text" data-toggle="tooltip" title="{{ $item['produto']->PRODUTO_DESC }}">
                                {{ strlen($item['produto']->PRODUTO_DESC) > 100 ? substr($item['produto']->PRODUTO_DESC, 0, 100) . '...' : $item['produto']->PRODUTO_DESC }}
                            </p>
                            <ul class="list-group list-group-flush">
                                <li class="list-group-item"><strong>R$
                                        {{ number_format($item['produto']->PRODUTO_PRECO, 2, ',', '.') }}</strong></li>
                                <li class="list-group-item">R$
                                    {{ number_format($item['produto']->PRODUTO_DESCONTO, 2, ',', '.') }} Descontos
                                </li>
                                <li class="list-group-item">Até 4x de R$
                                    {{ number_format($item['produto']->PRODUTO_PRECO / 4, 2, ',', '.') }}
                                </li>
                            </ul>
                            <p>Quantidade: {{ $item['quantidade'] }}</p>
                            <p>Total: R$
                                {{ number_format($item['produto']->PRODUTO_PRECO * $item['quantidade'], 2, ',', '.') }}
                            </p>
                            <form method="POST" action="{{ route('carrinho.remover') }}">
                                @csrf
                                <input type="hidden" name="produto_id" value="{{ $item['produto']->PRODUTO_ID }}">
